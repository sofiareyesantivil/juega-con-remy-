...
function draw() {
  ...
  switch (estadoActual) {
    ...
    case "MENU":
      ...
      let hoverF1 =
        mouseX >= 10 && mouseX <= 260 && mouseY >= 10 && mouseY <= 260;
      image(f1, 10, 10, hoverF1 ? 280 : 250, hoverF1 ? 280 : 250);

      let hoverT =
        mouseX >= 275 && mouseX <= 525 && mouseY >= 10 && mouseY <= 260;
      image(todito, 275, 10, hoverT ? 280 : 250, hoverT ? 280 : 250);

      let hoverQ =
        mouseX >= 540 && mouseX <= 790 && mouseY >= 10 && mouseY <= 260;
...
function dibujarSabores(ingrediente) {
  if (mouseIsPressed) {
    ...
    else if (ingrediente === "todo")
      colorTono = random(1) > 0.5 ? random(35, 60) : random(320, 360);
